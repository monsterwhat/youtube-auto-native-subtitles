...
4. Switches the subtitle **track only** — it never turns captions on or off. Your CC button stays yours. It waits for stable playback first, since switching mid-load breaks YouTube's caption module.
5. On multi-audio (dubbed) videos where subtitles still won't apply, switches the audio to the original-language track and retries — captions follow the playing audio.
